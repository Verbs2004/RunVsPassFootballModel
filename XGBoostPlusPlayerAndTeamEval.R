...
# For modeling - reduced feature set
df_modeling <- df %>%
  select(
...
# ───────────────────────────────────────────────────────────────
# 7) Train/test split and preprocessing - FIXED VERSION
# ───────────────────────────────────────────────────────────────
train_raw <- df_modeling %>% filter(season < 2023)
test_raw <- df_modeling %>% filter(season == 2023)

# Keep game_seconds_remaining for interaction features
...
create_interactions <- function(data) {
  data %>%
    ...
    # Remove game_seconds_remaining AFTER creating interactions
