...
```{r}
# ── 0) Package bootstrap ---------------------------------------------------
pkgs <- c("ggplot2", "scales", "dplyr", "nflreadr", "nflplotR")  # ggrepel no longer needed
missing <- pkgs[!(pkgs %in% rownames(installed.packages()))]
if (length(missing)) install.packages(missing, dependencies = TRUE)
invisible(lapply(pkgs, library, character.only = TRUE))
# --------------------------------------------------------------------------

# ── 1) Data (your existing tibble) ----------------------------------------
team_props <- data.frame(
  team = c("ARI","ATL","BAL","BUF","CAR","CHI","CIN","CLE","DAL","DEN","DET",
           "GB","HOU","IND","JAX","KC","LA","LAC","LV","MIA","MIN","NE","NO",
           "NYG","NYJ","PHI","PIT","SEA","SF","TB","TEN","WAS"),
  offense_run_prop = c(
    0.3409490,0.5459883,0.4584139,0.3347548,0.4082969,0.5352941,0.3520871,
    0.4785156,0.4701987,0.3877551,0.4104167,0.3854545,0.3958333,0.3420139,
    0.4272560,0.3074004,0.3476395,0.3186373,0.3432836,0.3500967,0.3581781,
    0.4450758,0.3992740,0.4669510,0.3694853,0.4674556,0.3465553,0.4034707,
    0.4317181,0.3065954,0.5476190,0.3697183
  ),
  defense_run_prop = c(
    0.3700935,0.3641115,0.3227017,0.3697105,0.4532374,0.4688129,0.4250936,
    0.4320175,0.4291755,0.3721881,0.4036511,0.4897119,0.4846626,0.4495238,
    0.3992467,0.3447581,0.3928571,0.4123932,0.4089069,0.3677298,0.3737166,
    0.3938294,0.4157088,0.3775281,0.3978495,0.3492723,0.3968566,0.4137931,
    0.4219512,0.4066543,0.2898551,0.4022770
  )
)

# ── 2) Plot with logos -----------------------------------------------------
ggplot(team_props,
       aes(x = offense_run_prop, y = defense_run_prop)) +
  # ← nflplotR grabs the ESPN-style SVG/PNG URLs that live inside nflreadr::load_teams()
  nflplotR::geom_nfl_logos(aes(team_abbr = team), width = 0.06) +
  geom_hline(yintercept = mean(team_props$defense_run_prop),
             linetype = "dashed", colour = "grey60") +
  geom_vline(xintercept = mean(team_props$offense_run_prop),
             linetype = "dashed", colour = "grey60") +
  ...
  labs(subtitle = "Dashed lines represent league average") +
...
# ---- Package bootstrap (readr, dplyr, stringr, plotly) --------------------
pkgs <- c("readr", "dplyr", "stringr", "plotly")
...
fig
```
## A taste of more stuff we've worked on {.smaller}
